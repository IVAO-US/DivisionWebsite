...
<div>
    ...
    <x-card title="Active Admins" shadow separator class="mb-8">
        ...
        {{-- Admins List Table --}}
        <livewire:protected.admin.admins_list-table :search="$search" wire:key="admin-search-{{ $search }}" />
    </x-card>
        ...
        <x-card subtitle="Search User" shadow separator>
            ...
            <div class="mb-6">
                ...
                @if(strlen($userSearch) >= 3)
                    <livewire:protected.admin.membership.users_list-table :search="$userSearch" wire:key="user-search-{{ $userSearch }}" />
                @else
